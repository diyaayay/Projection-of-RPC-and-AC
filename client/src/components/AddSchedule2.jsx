...
const AddSchedule2 = ({ count, onSave, onCancel, count_onprim }) => {

    function getRandomInt(max) {
        return Math.floor(Math.random() * max);
    }
    const randomCount = getRandomInt(count_onprim) + 1;



    const [scheduleId] = useState(count)
    const [scheduleName] = useState(`On-Premises_Protection_Store_${count}`);
